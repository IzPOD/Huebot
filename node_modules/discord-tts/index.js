...
function allBase64ToBinary(results) {
  let binary = "";
  
  results.forEach(function(item, i, arr) {
    binary += Buffer.from(item.base64, "base64").toString("binary");
  });
...
function allBase64toBinaryStream(base64Text){
  const binary=allBase64ToBinary(base64Text);
  const stream=new Stream.PassThrough();
  stream.write(binary,"binary");
  return stream;
}
...
function downloadFromInfoCallback(stream, text, {lang, slow, host, timeout, splitPunct}) {
    googleTTS.getAllAudioBase64(text, {lang, slow, host, timeout, splitPunct})
      .then(result => {let temp = allBase64toBinaryStream(result); console.log(result); return temp})
      .then(audioStream => {audioStream.pipe(stream); audioStream.end("\0");})
      .catch(console.error);
}
 ...
 * @param {string} cfg.splitPunct
 */
function getVoiceStream(text, {lang = 'ru', slow = false, host = 'https://translate.google.com', timeout = 10000, splitPunct} = {}) {
    const stream = new Stream.PassThrough();
